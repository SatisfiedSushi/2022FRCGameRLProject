...
class env(gym.Env):
    ...
    def __init__(self, render_mode="human"):
        ...
        self.observation_space = Dict({
            agent: Box(low=np.array([-1, -1, -1, 0, -180, -180, -180, -180, -180, -180, 0, 0, 0, 0, 0]), high=np.array([1, 1, 1, 360, 180, 180, 180, 180, 180, 180, 3, 3, 3, 3, 3]), shape=(15,))
    ...
    def step(self, actions):  # TODO: change action dictionary
        ...
        obs = {
            agent: [self.swerve_instances[self.agents.index(agent)].get_velocity()[0], self.swerve_instances[self.agents.index(agent)].get_velocity()[1],
                    ...
                    self.return_closest_ball(self.swerve_instances[self.agents.index(agent)].get_box2d_instance())[1],
